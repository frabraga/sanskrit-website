...
module.exports = {
  async up() {
    ...
    // Strapi runs custom migrations BEFORE syncing schema changes, so new columns
    // added to schema.json may not exist yet in the DB. Ensure `commentary` exists.
    const db = strapi.db.connection;
    const hasCommentary = await db.schema.hasColumn("shabdas", "commentary");
    if (!hasCommentary) {
      console.log("⚙️  Adding missing 'commentary' column to shabdas...");
      await db.schema.table("shabdas", (t) => t.text("commentary").nullable());
    }
